...
class CharacterManager:
    ...
    def roles(self, text=""):
        return [
            ...
            if c.role.lower().startswith(text)
        ]
    ...
    def do_player_sheet(self, arg):
        ...
        print(
            f"HANDLE \033[1;3;35m{self.char_mngr.player.handle:⌁^33}"
            "\033[0m ROLE "
            f"\033[1;3;35m{self.char_mngr.player.role:⌁^33}\033[0m"
        )
        stat_list = [
            (
                f"{key:⌁<12}{self.char_mngr.player.lucky_pool}/{value}"
                if key == "luck"
        ...
        ware_list = [
            value
            for row in self.char_mngr.player.cyberware
