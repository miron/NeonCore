...
class BrawlingShell(AsyncCmd):
    ...
    async def do_grab(self, arg):
        ...
        grapple = GrappleShell(self.player, self.target.handle, self.io)
        await grapple.cmdloop()
...
class GrappleShell(AsyncCmd):
    ...
    def __init__(self, player, target_name, io):
        super().__init__(io)
        self.player = player
    ...
    async def do_choke(self, arg):
        ...
        await self.io.send(f"\033[31m[PLACEHOLDER] Choking {self.target_name}...\033[0m")
    ...
    async def do_throw(self, arg):
        ...
        await self.io.send(f"\033[31m[PLACEHOLDER] Throwing {self.target_name}!\033[0m")
        return True
