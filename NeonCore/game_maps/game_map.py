...
class Map:
    ...
    def do_move(self, args):
        ...
        my_map = Map(self.char_mngr.get_player(), self.char_mngr.get_npcs())
        my_map.move()
    ...
    def move(self):
        while True:
            ...
            if not (
                self.is_within_map_bounds(new_x, new_y)
                and self.is_valid_move(new_x, new_y)
            ):
                continue
    ...
    def is_within_map_bounds(self, x, y):
        return (
            x >= 0 and x < len(self.map_data[0]) and y >= 0 and y < len(self.map_data)
        )
